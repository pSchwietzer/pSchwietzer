<div align="center">

<img src="https://gitlab.com/pSchwietzer/pSchwietzer/-/raw/main/images/ellen.jpg" width="100%" alt="ZZZ Ellen Joe" />

### My Projects

<table>
    <tr>
        <th><div align="center">Name</div></th>
        <th width="100%"><div align="center">Description</div></th>
    </tr>
    <tr>
        <td>
            <div align="center">
                <a target=”_blank” href="https://gitlab.com/pSchwietzer/suukon">Suukon</a>
            </div>
        </td>
        <td>
            <div align="Left">CLI and Library for converting numbers between different numeral systems written in Rust.</div>
        </td>
    </tr>
        <tr>
        <td>
            <div align="center">
                <a target=”_blank” href="https://gitlab.com/pSchwietzer/full2half">Full2Half</a>
            </div>
        </td>
        <td>
            <div align="Left">Library and CLI for converting full-width characters to half-width characters and vice versa, written in Rust.</div>
        </td>
    </tr>
    <tr>
        <td>
            <div align="center">
                <a target=”_blank” href="https://gitlab.com/pSchwietzer/arasaka-plymouth">Arasaka Plymouth</a>
            </div>
        </td>
        <td>
            <div align="left">A plymouth theme for the boot screen themed around Arasaka from Cyberpunk 2077.</div>
        </td>
    </tr>
    <tr>
        <td>
            <div align="center">
                <a target=”_blank” href="https://gitlab.com/pSchwietzer/arasaka-dot">ARASAKA.DOT</a>
            </div>
        </td>
        <td>
            <div align="left">My dot files themed around Arasaka from Cyberpunk 2077.</div>
        </td>
    </tr>
</table>

<br />

<a target=”_blank” href="https://gitlab.com/pSchwietzer">
<img src="https://gitlab.com/pSchwietzer/pSchwietzer/-/raw/main/images/social/gitlab.svg" alt"Gitlab" height="48" width="48" />
</a>
&nbsp;&nbsp;
<a target=”_blank” href="https://github.com/pSchwietzer">
<img src="https://gitlab.com/pSchwietzer/pSchwietzer/-/raw/main/images/social/github.svg" alt"Github" height="48" width="48" />
</a>
&nbsp;&nbsp;
<a target=”_blank” href="https://linkedin.com/in/pschwietzer">
<img src="https://gitlab.com/pSchwietzer/pSchwietzer/-/raw/main/images/social/linkedin.svg" alt"LinkedIn" height="48" width="48" />
</a>

</div>
